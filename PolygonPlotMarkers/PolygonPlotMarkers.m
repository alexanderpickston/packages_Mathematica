...
AllShapes::usage="Table with all the available shapes."
FilledMarker::usage="Filled marker"
EmptyMarker::usage="Empty marker"
CustomFilledMarker::usage="Make filled marker from string"
CustomEmptyMarker::usage="Make empty marker from string"
...
AllShapes[]:=
TableForm[Transpose[{PolygonMarker[All],Tooltip[Graphics[{FaceForm[Hue@Random[]],EdgeForm[{Black,Thickness[0.003],JoinForm["Miter"]}],PolygonMarker[#,1]},ImageSize->30,PlotRange->1.5,PlotRangePadding->0,ImagePadding->0],#]&/@PolygonMarker[All]}]]

FilledMarker[name_,size_: 7]:=Graphics[{EdgeForm[],PolygonMarker[name,Offset[size]]},AlignmentPoint->{0,0}]
EmptyMarker[name_, size_: 7] := 
 Graphics[{Dynamic@
...
MarkerizeString[l_String] := 
  First@Cases[
    ImportString[
     ExportString[Style[l, FontFamily -> "Verdana", FontSize -> 20], 
      "PDF"], "PDF"], c_FilledCurve :> c[[2, 1]], Infinity];

CustomFilledMarker[l_String,size_: 0.05]:=Graphics[{FaceForm[Black],EdgeForm[None],PolygonMarker[MarkerizeString[l],Scaled[size]]},AlignmentPoint->{0,0}]

CustomEmptyMarker[l_String,size_: 0.05]:=Graphics[{Dynamic@
    EdgeForm@Directive[CurrentValue["Color"], JoinForm["Round"], AbsoluteThickness[2], 
      Opacity[1]], FaceForm[White], PolygonMarker[MarkerizeString[l],Scaled[size]]}, 
  AlignmentPoint -> {0, 0}]
